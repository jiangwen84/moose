...
#include "C4ZrICConstExpcomp.h"
#include "MooseUtils.h"

registerMooseObject("MooseApp", C4ZrICConstExpcomp);

InputParameters
C4ZrICConstExpcomp::validParams()
{
  ...
  params.addClassDescription(
      "Initial condition for the C4 model for high temperature "
      "corrosion of Zircaloy-4. The profile is initiated as constant in each phase.");
  return params;
}

C4ZrICConstExpcomp::C4ZrICConstExpcomp(const InputParameters & parameters)
  : InitialCondition(parameters), _temperature(getParam<Real>("temperature"))
{
  /**  if (MooseUtils::absoluteFuzzyEqual(_temperature,1273.15,1))
    ...
    0.84697*_temperature + 977.01  ; _x_ox_a = -3.6071*1e-5*pow(_temperature,2)
    + 7.1427*1e-2*_temperature + 563.11  ;
    }
    */
...
Real
C4ZrICConstExpcomp::value(const Point & p)
{
  if (p(0) < _x_a_b)
  {
    return _C_b;
