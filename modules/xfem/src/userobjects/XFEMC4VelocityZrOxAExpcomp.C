...
#include "XFEMC4VelocityZrOxAExpcomp.h"
#include "MooseUtils.h"

registerMooseObject("XFEMApp", XFEMC4VelocityZrOxAExpcomp);

template <>
InputParameters
validParams<XFEMC4VelocityZrOxAExpcomp>()
{
...
XFEMC4VelocityZrOxAExpcomp::XFEMC4VelocityZrOxAExpcomp(const InputParameters & parameters)
  : XFEMMovingInterfaceVelocityBase(parameters), _temperature(getParam<Real>("temperature"))
{
}

Real
XFEMC4VelocityZrOxAExpcomp::computeMovingInterfaceVelocity(unsigned int point_id) const
{
  ...
  Real delta = zirconium_PBR * std::abs(xt - 1250);
