...
#include "XFEMC4VelocityZrOxAExample.h"
#include "MooseUtils.h"

registerMooseObject("XFEMApp", XFEMC4VelocityZrOxAExample);

template <>
InputParameters
validParams<XFEMC4VelocityZrOxAExample>()
{
...
XFEMC4VelocityZrOxAExample::XFEMC4VelocityZrOxAExample(const InputParameters & parameters)
  : XFEMMovingInterfaceVelocityBase(parameters), _temperature(getParam<Real>("temperature"))
{
}

Real
XFEMC4VelocityZrOxAExample::computeMovingInterfaceVelocity(unsigned int point_id) const
{
  ...
  Real delta = zirconium_PBR * std::abs(xt - 1000);
