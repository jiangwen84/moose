...
WeightGainSpaceIntegralZr::validParams()
{
  ...
  params.addRequiredParam<PostprocessorName>(
      "concentration_integral", "The name of the oxygen concentration integral postprocessor");
  params.addParam<Real>("ymax", 4, "The mesh y dimension [um]");
  params.addParam<Real>("zmax", 4, "The mesh z dimension [um]");
  params.addRequiredParam<PostprocessorName>("oxide_thickness",
                                             "The name of the oxide thickness postprocessor");
  params.addRequiredParam<PostprocessorName>("alpha_thickness",
                                             "The name of the alpha layer thickness postprocessor");
  return params;
}
...
WeightGainSpaceIntegralZr::execute()
{
  ...
  const Real Czro2 =
      1e6 * rho_ZrO2 * Na / (Mzr + 2 * Mo); // Concentration of Zr atoms in the oxide[/m³]
  const Real Zr_PBR(1.55);

  // First, convert the integrated concentration in /m² (divide by ymax to cancel integration over
  // y)
  ...
  Real x_b_a = (9.59e-3 * (_temperature - 1136) + 4.72e-6 * pow(_temperature - 1136, 2) -
                4.35e-9 * pow(_temperature - 1136, 3)) *
               1e-2;
  Real x_a_b = (45.86e-3 * (_temperature - 1136) - 44.77e-6 * pow(_temperature - 1136, 2) +
  ...
  Real oxide_weak_integral =
      1e-6 * _delta / Zr_PBR * (C_a_ox - (C_a_b - C_b_a) + 0.5 * (C_ox - C_ox_a));

  // And from that the metal part only of the weak integral [/m²]:
  Real metal_weak_integral = full_weak_integral - oxide_weak_integral;

  // The strong part of the integral in the metal, that was not accounted for during integration
  // [/m²]:
  ...
  // Thus, the integral of the oxygen concentration that has came into the sample during corrosion
  // is [/m²]:
  Real total_integral =
      metal_weak_integral + metal_strong_missing + oxide_strong_integral - metal_native_integral;
  ...
  // Finally, we can get the weight gain resulting from space integration of the oxygen
  // concentration [mg/cm²]:
